    ...
    <div role="radiogroup" aria-label="Presentation mode" className={cn("relative flex rounded-full p-1", dark ? "bg-white/10 ring-1 ring-white/15" : "bg-white ring-1 ring-line")}>
      {OPTIONS.map((o) => {
        ...
        return (
          ...
          >
            {active && <motion.span layoutId="presentation-pill" className={cn("absolute inset-0 -z-10 rounded-full", dark ? "bg-white" : "bg-navy")} transition={{ type: "spring", stiffness: 500, damping: 38 }} />}
